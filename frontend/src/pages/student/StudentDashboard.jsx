...
export default function StudentDashboard({ onNavigate, currentUser }) {
  ...
  return (
    <div className="student-page">
      ...
      {/* Risk Status Summary Card */}
      <div className="card student-risk-banner">
        <div className="risk-banner-content">
          <div className="risk-banner-icon">🛡️</div>
          <div>
            <h3>Academic Standing & Risk Indicator</h3>
            <p className="risk-status-text">
              Status: <span className="risk-badge-normal">{data?.riskStatus || 'Good Standing (ML Integration Pending)'}</span>
            </p>
            <p className="risk-notice">
              Predictive risk assessment and intelligent early warning will activate when the AI Advisor is integrated.
            </p>
          </div>
        </div>
        <button className="btn btn-secondary" onClick={() => onNavigate('risk')} style={{ width: 'auto', alignSelf: 'center' }}>
          View Details
        </button>
      </div>
